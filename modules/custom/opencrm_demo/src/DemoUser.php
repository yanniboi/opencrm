...
abstract class DemoUser extends DemoContent {
   ...
   * @param \Drupal\opencrm_demo\DemoContentParserInterface $parser
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static(
      ...
      $plugin_id,
      $plugin_definition,
      $container->get('opencrm_demo.yaml_parser'),
...
//        drush_log(dt("User with uuid: {$uuid} already exists."), LogLevel::WARNING);
      ...
      if (empty($item['roles'])) {
        $item['roles'] = [AccountInterface::ANONYMOUS_ROLE];
      }
...
//    $profile->field_profile_image = $item['picture'];
    $profile->crm_name = $item['first_name'];
...
//    $profile->field_profile_function = $item['function'];
    $profile->crm_phone = $item['phone_number'];
