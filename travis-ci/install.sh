...
# Add our repositories for opencrm, as well as re-adding the Drupal package repo.
echo "# Configuring package repos"
composer config repositories.0 path $TRAVIS_BUILD_DIR
composer config repositories.1 composer https://packages.drupal.org/8
composer config extra.enable-patching true

# Now require opencrm which will pull itself from the paths.
echo "# Requiring opencrm"
composer require drupal/opencrm dev-master